...
@Composable
fun CallTypeFormScreen(
    viewModel: FormViewModel = hiltViewModel(), // Hilt-provided ViewModel,
    onContinue: () -> Unit
) {

    var title by remember { mutableStateOf("Call Type") }
    LaunchedEffect(Unit) {
        val headersList = viewModel.loadHeadersList()
        title = headersList.takeIf { it.size > 1 }?.get(1) ?: "Call Type"
    }
    ...
    Column(
        ...
        Button(
            onClick = {
                viewModel.updateCallType("FOC")
                onContinue()
            },
        ) {
            Text("FOC ( free of charge)")
        }
        ...
        Button(
            ...
            onClick = {
                viewModel.updateCallType("Chargeable")
                onContinue()
            },
        ) {
            Text("Chargeable")
