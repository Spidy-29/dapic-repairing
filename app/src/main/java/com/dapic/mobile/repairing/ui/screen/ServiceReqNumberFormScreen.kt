...
@Composable
fun ServiceReqNumberFormScreen(
    viewModel: FormViewModel = hiltViewModel(), // Hilt-provided ViewModel,
    onContinue: () -> Unit
) {
    var title by remember { mutableStateOf("Service Req Number") }
    LaunchedEffect(Unit) {
        val headersList = viewModel.loadHeadersList()
        title = headersList.takeIf { it.size > 5 }?.get(5) ?: "Service Req Number"
    }

    val formState by viewModel.formState.collectAsState()
//    val serviceRequestNumber by viewModel.selectedServiceRequestCode.collectAsState()
